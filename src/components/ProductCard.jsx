    ...
    return (
        <Link to={`/listing/${id}`} className="rounded-md overflow-hidden min-h-44 md:min-h-56 hover:-translate-y-1 hover:shad bg-white group">
            ...
            <div className="flex flex-col justify-between px-4 pb-4 pt-2">
                <h5 className="text-slate-700 text-base sm:text-sm text-justify leading-tight font-semibold">{title}</h5><p className="flex items-center font-medium text-xs sm:text-sm opacity-70 text-slate-600"> <TbMapPinPin className="mr-1 text-inherit" />{address}</p>
            </div>
            {
                full ? <div className="flex items-center justify-between p-2 md:p-4 bg-white group-hover:bg-orange-500 text-slate-700 group-hover:text-white">
