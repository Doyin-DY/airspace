...
export default function ProductCard({
  ...
  return (
    <Link
      to={`/listing/${id}`}
      className="rounded-md overflow-hidden min-h-44 md:min-h-56 hover:-translate-y-1 hover:shad group"
    >
      <div className="h-32 w-full overflow-hidden relative group">
        <div className="bg-slate-700 group-hover:bg-orange-500  py-1 px-4 w-32 rounded-2xl text-sm sm:text-sm text-white text-center absolute z-10 top-4 left-3">
      ...
      <div className="flex flex-col justify-between px-4 pb-4 pt-2">
        <h5 className="text-slate-700 text-base font-semibold sm:text-lg text-justify leading-tight">
          {title}
        </h5>
        <p className="flex items-center font-medium text-xs  sm:text-sm opacity-70 text-slate-600">
          <TbMapPinPin className="mr-1 text-inherit" />
          {address}
        </p>
      </div>
